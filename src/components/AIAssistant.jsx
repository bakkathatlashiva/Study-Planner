...
export default function AIAssistant({ isActive, setCurrentScreen, addXP }) {
  ...
  const handleSendAIChat = async () => {
    ...
    setAiChat((prev) => [
      ...prev,
      { role: "bot", text: "Thinking...", loading: true },
    ]);
    ...
    try {
      const resp = await callClaude(
        systemPrompts[aiMode] || systemPrompts.explain,
        text,
        800,
      );
    ...
    <div id="ai-screen" className={`screen ${isActive ? "active" : ""}`}>
      ...
      <div className="ai-chat">
        {aiChat.map((m, idx) => (
          <div
            key={idx}
            className={`ai-msg ${m.role} ${m.loading ? "loading" : ""}`}
          >
            {m.loading ? (
              <>
      ...
      <div className="ai-input-wrap">
        <div className="mode-chips">
          {[
            ...
            <div
              key={chip.id}
              className={`chip ${aiMode === chip.id ? "active" : ""}`}
              onClick={() => setAiMode(chip.id)}
            >
              {chip.label}
            </div>
          ))}
        ...
        <div className="ai-row">
          <input
            ...
            placeholder={(() => {
              const hints = {
                explain: "Enter topic to explain...",
                mock_test: "Enter subject and units...",
                daily_coach: "How are you feeling today?",
                flashcards: "Enter topic for flashcards...",
                debug: "Paste your code/error...",
                roadmap: "Enter your career goal...",
                weakness_killer: "Enter your weak topics...",
                exam_readiness: "Enter your subject...",
                placement_mode: "Enter topic (DSA/SQL etc.)...",
          ...
          />
          <button className="ai-send" onClick={handleSendAIChat}>
            ➤
          </button>
        </div>
      </div>
